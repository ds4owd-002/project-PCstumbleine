...
# cap the y-axis just above the widest whisker across tasks, rounded up to
# the nearest 30 seconds, so a handful of extreme outliers don't compress
# the readable range for every other task
task_fences <- gulper_data %>%
  group_by(task) %>%
  summarise(
    q1 = quantile(duration_seconds, 0.25),
    q3 = quantile(duration_seconds, 0.75),
    fence = q3 + 1.5 * (q3 - q1),
    upper_whisker = max(duration_seconds[duration_seconds <= fence])
  )

axis_upper_seconds <- ceiling(max(task_fences$upper_whisker) / 30) * 30

ggplot(gulper_data, aes(x = fct_reorder(task, duration_seconds, .fun = median), y = duration_seconds, fill = task)) +
  geom_boxplot(show.legend = FALSE) +
  geom_text(
    data = task_medians,
    aes(x = fct_reorder(task, median_sec), y = q3_sec, label = paste0(round(median_sec), "s")),
    inherit.aes = FALSE,
    nudge_x = 0.35,
    size = 3.5
  ) +
  coord_cartesian(ylim = c(0, axis_upper_seconds)) +
  scale_fill_viridis_d() +
  scale_y_continuous(breaks = seq(0, axis_upper_seconds, by = 30)) +
  labs(
    x = "Task category",
    y = "Duration (seconds)",
    title = "Spread of task durations"
  ) +
  theme_minimal()
```

@fig-duration-boxplot shows the full spread of task durations rather than just a single summary bar: Earthworks and Extraction have both a high median and a wide interquartile range, while Communication is short with a narrow, tightly clustered range. Because the median line sits inside each box rather than at either end, it is clear the distributions are right-skewed, with each task's median well below the upper whisker. A handful of unusually long tasks (up to about 25 minutes for Earthworks) sit above the axis limit shown here; the full range, including those extremes, is reported in @tbl-task-summary.
