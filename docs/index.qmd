...
#| fig-cap: "Percentage of each job's total observed time spent on each task category, shown for the three individual ridealong sessions plus a fourth 'All jobs' column combining all three."

job_labels <- gulper_data %>%
  mutate(job_date = as.character(job_date))

job_share_data <- bind_rows(
  job_labels,
  job_labels %>% mutate(job_date = "All jobs")
) %>%
  mutate(job_date = factor(job_date, levels = c(sort(unique(job_labels$job_date)), "All jobs")))

ggplot(job_share_data, aes(x = job_date, y = duration_seconds, fill = task)) +
...
#| fig-cap: "Median duration of a task interval, by task category, on an ordinary (linear) scale. Bars are labelled with the exact median duration in seconds."
...
ggplot(task_duration_typical, aes(x = fct_reorder(task, median_sec), y = median_sec, fill = task)) +
  geom_col(show.legend = FALSE) +
  geom_text(aes(label = paste0(round(median_sec), "s")), hjust = -0.15, size = 3.5) +
  coord_flip(clip = "off") +
  scale_fill_viridis_d() +
  scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +
  labs(
    x = "Task category",
    y = "Median task-interval duration (seconds)",
    title = "How long a typical interval of each task takes"
  ) +
  theme_minimal()
```

@fig-typical-duration shows on an ordinary linear scale just how much longer a typical Earthworks interval is than a typical Communication interval: about 98 seconds versus roughly 15 seconds, a roughly six-fold difference that a log scale can visually understate. Median rather than mean duration is plotted here because a handful of unusually long intervals (up to about 25 minutes for Earthworks) would otherwise pull the bars up and obscure the typical case; the full spread, including those outliers, is reported in @tbl-task-summary.
