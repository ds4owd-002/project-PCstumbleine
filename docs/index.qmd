...
## Total time per task, job by job

```{r}
#| label: fig-task-totals-by-job
#| fig-cap: "Total time spent on each task category, shown separately for each of the three ridealong jobs. Each task category has one point per job; colour and shape both encode the job so the figure stays readable for colour-blind viewers or in greyscale."

job_dates <- sort(unique(as.character(gulper_data$job_date)))

job_task_totals <- gulper_data %>%
  mutate(job_date = as.character(job_date)) %>%
  group_by(job_date, task) %>%
  summarise(total_minutes = sum(duration_seconds) / 60, .groups = "drop") %>%
  mutate(job_label = paste0("Job ", match(job_date, job_dates)))

axis_upper_minutes <- ceiling(max(job_task_totals$total_minutes) / 30) * 30

ggplot(
  job_task_totals,
  aes(
    x = fct_reorder(task, total_minutes, .fun = median),
    y = total_minutes,
    colour = job_label,
    shape = job_label
  )
  ...
  scale_y_continuous(breaks = seq(0, axis_upper_minutes, by = 30)) +
  labs(
    x = "Task category",
    y = "Total time spent on task (minutes)",
    colour = "Job",
    shape = "Job",
    title = "Total time per task, job by job"
  ) +
  theme_minimal()
```

@fig-task-totals-by-job shows that Earthworks time climbs fairly steadily across the three jobs (68, 76 and 119 minutes), while other tasks swing far more erratically: Rest jumps from about 5 minutes on Job 2 to over 180 minutes on Job 3, and both Extraction and Fluidizing are far higher on Job 2 than on Job 1. Some of this is simply because Job 3 was a much longer session overall, but the uneven scaling across tasks - Rest growing far more than Earthworks between jobs, for example - suggests that job length alone does not explain how gulpers' time gets allocated across task categories.
