...
task_medians <- gulper_data %>%
  group_by(task) %>%
  summarise(median_sec = median(duration_seconds))

# cap the y-axis just above the widest whisker across tasks, rounded up to
# the nearest whole minute, so a handful of extreme outliers don't compress
...
axis_upper_seconds <- ceiling(max(task_fences$upper_whisker) / 60) * 60

ggplot(gulper_data, aes(x = fct_reorder(task, duration_seconds, .fun = median), y = duration_seconds, fill = task)) +
  geom_boxplot(show.legend = FALSE) +
  geom_label(
    data = task_medians,
    aes(x = fct_reorder(task, median_sec), y = median_sec, label = paste0(round(median_sec), "s")),
    inherit.aes = FALSE,
    nudge_x = 0.42,
    size = 3.8,
    fontface = "bold",
  ...
  scale_fill_viridis_d() +
  scale_y_continuous(
    breaks = seq(0, axis_upper_seconds, by = 60),
